...
export default function Dashboard() {
  ...
  const { posts, deletePost, publishPost, unpublishPost } = useBlogStore()
  ...
  function handleDelete(id: string, title: string) {
    if (confirm(`确定要删除《${title}》吗？此操作不可撤销。`)) deletePost(id)
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: '全部文章', value: posts.length, icon: FileText, color: 'blue' },
        ...
        {([['all', '全部'], ['published', '已发布'], ['draft', '草稿']] as const).map(([val, label]) => (
          ...
          >
            ...
            <span className={`ml-1.5 text-xs ${tab === val ? 'text-blue-600' : 'text-gray-400'}`}>
              {val === 'all' ? posts.length : val === 'published' ? publishedCount : draftCount}
            </span>
          </button>
        ))}
      </div>
