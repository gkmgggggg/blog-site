...
export default function PostDetail() {
  ...
  const { getPost, incrementViews, posts } = useBlogStore()
  ...
  if (!post) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-500 mb-6">该文章可能已被删除或链接有误</p>
      ...
      <article>
        ...
        <header className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 pb-6 border-b border-gray-100">
            ...
            <Link
              to={`/editor/${post.id}`}
              className="ml-auto flex items-center gap-1.5 text-blue-600 hover:underline"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {related.map((p) => (
              <Link key={p.id} to={`/post/${p.slug}`} className="group bg-white rounded-xl p-4 border border-gray-100 hover:border-blue-200 hover:shadow-sm transition-all">
                {p.coverImage && (
                  <img src={p.coverImage} alt={p.title} className="w-full h-32 object-cover rounded-lg mb-3" />
                )}
