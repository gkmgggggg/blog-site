...
async function request<T>(url: string, init: RequestInit = {}, auth = false): Promise<T> {
  ...
  const res = await fetch(url, { ...init, headers: { ...h, ...(init.headers as Record<string, string>) } })
...
function mapPost(p: Record<string, unknown>): Post {
  ...
  return {
    ...
    author: author
      ? { id: author.id as string, name: author.name as string, avatar: author.avatar as string, bio: (author.bio as string) || '' }
      : { id: '', name: '', avatar: '', bio: '' },
...
async function listPosts(query: PostsQuery = {}): Promise<PostsResponse> {
  ...
  return {
    posts,
    meta: { page: m.page as number, pageSize: m.page_size as number, total: m.total as number, totalPages: m.total_pages as number },
  }
}
...
export const postsApi = {
  ...
  create: async (data: Partial<Post>): Promise<Post> => {
    ...
    const result = await request<Record<string, unknown>>('/api/posts', { method: 'POST', body: JSON.stringify(body) }, true)
  ...
  update: async (id: string, data: Partial<Post>): Promise<Post> => {
    ...
    const result = await request<Record<string, unknown>>(`/api/posts/${id}`, { method: 'PUT', body: JSON.stringify(body) }, true)
    return mapPost(result)
  },

  delete: (id: string): Promise<unknown> =>
    request(`/api/posts/${id}`, { method: 'DELETE' }, true),

  publish: async (id: string): Promise<Post> => {
    const result = await request<Record<string, unknown>>(`/api/posts/${id}/publish`, { method: 'PATCH' }, true)
    return mapPost(result)
  },

  unpublish: async (id: string): Promise<Post> => {
    const result = await request<Record<string, unknown>>(`/api/posts/${id}/unpublish`, { method: 'PATCH' }, true)
    return mapPost(result)
  },
  ...
  archives: (): Promise<ArchiveYear[]> =>
    request<{ year: number; posts: Record<string, unknown>[] }[]>('/api/posts/archives')
      .then((data) => data.map((y) => ({ year: y.year, posts: y.posts.map(mapPost) }))),
}
